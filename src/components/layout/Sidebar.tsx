...
const Sidebar = ({ isOpen, onClose }: SidebarProps) => {
  ...
  const userNavigation: SidebarItem[] = [
    ...
    },
    {
      ...
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-5 5v-5zM15 17H9a4 4 0 01-4-4V7a4 4 0 014-4h6a4 4 0 014 4v6a4 4 0 01-4 4z" />
        </svg>
      ),
    },
  ];
      ...
      <div key={item.name}>
        ...
        >
          <div className="flex items-center">
            <span className={`${isActive ? 'text-primary-500' : 'text-gray-400'} mr-3`}>
              {item.icon}
            </span>
            <span>{item.name}</span>
          </div>
